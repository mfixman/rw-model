...
# Initially, alpha_mack and alpha_hall are identical.
    ...
    def get_alpha_hall(self, cs, sigma, lamda):
        ...
        try:
            ...
            print("HALL")
            print(1-lamda+sigma)
            error = (((1-abs(lamda-sigma)) * self.alpha_hall[cs] * (1-self.xi_hall*math.exp(- delta_ma_hall**2 / 2)))+abs(lamda-sigma))/2
        except:
            error = ((1-abs(lamda-sigma))*self.alpha_hall[cs] + abs(lamda-sigma))/2

        print(f"exp: {(1-self.xi_hall*math.exp(- delta_ma_hall**2 / 2))}:.4f")
